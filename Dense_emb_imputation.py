...
from keras.layers import Input, Dense, Softmax, Reshape, Embedding, Conv1D, GlobalAveragePooling1D, Flatten, Multiply, Lambda
...
app = typer.Typer(
    ...
    help=""" Dense Autoencoder for SNP Imputation with Embedding

 python Dense_emb_imputation.py --help

 python Dense_emb_imputation.py train --help

 python Dense_emb_imputation.py eval --help

 python Dense_emb_imputation.py train-and-eval --help
...
class SNP_Autoencoder:
    def __init__(self, num_snps, mask_miss=None,  weight=0.0, weight_0=1.0, weight_1=1.0, weight_2=1.0, lr=1e-4,
                 ...
                 export_model_name="DenseEmbAE", mult_1=2.3, mult_2=2.6, pool_type="attention"):
    ...
    def build_model(self):
        ...
        # embed categorical genotypes 0/1/2:
        # each genotype value is replaced by a learnable 3-dimensional vector.
        x = Embedding(input_dim=3, output_dim=3)(inp)

        # configurable pooling
        if self.pool_type == "flatten":
            # keeps SNP structure explicitly, no explicit selection, poor scalability for large num_snps
            x = Conv1D(self.hidden_dim, kernel_size=3, strides=2, activation=self.activation)(x)
            x = Conv1D(self.hidden_dim, kernel_size=3, strides=2, activation=self.activation)(x)
            x = Flatten()(x)

        elif self.pool_type == "average":
            # every SNP contributes equally, regardless of relevance
            x = Conv1D(self.hidden_dim, kernel_size=1, activation=self.activation)(x)
            x = GlobalAveragePooling1D()(x)

        elif self.pool_type == "attention":
            # each SNP embedding (length 3) -> hidden_dim features
            x = Conv1D(self.hidden_dim, kernel_size=1, activation=self.activation)(x)
            # attention score computation
            attn = Dense(1)(x) # each SNP = one scalar score (how important that SNP is, given its embedded genotype)
            # attention normalization (attention weights for all SNPs in one sample sum to 1)
            # = SNPs compete with each other for importance
            attn = Softmax(axis=1)(attn)    
            # weighting SNP representations
            # each SNP’s feature vector is multiplied by its attention weight (important SNPs are amplified)
            x = Multiply()([x, attn])
            # pooling by weighted sum (many SNP-level vectors are collapsed into one vector per sample)
            x = Lambda(lambda t: tf.reduce_sum(t, axis=1))(x)

        else:
            raise ValueError("pool_type must be 'flatten', 'average', or 'attention'")
    ...
    def train(self, x, batch_size, val_split, model_path=None, epochs=1000, results_dir=None, save_means_path=None):
        ...
        name = Path(model_path).stem if model_path else "dense_emb_ae"

        if results_dir is None:
            result_dir = Path("results") / f"model_DenseEmbAE_{timestamp}_{name}"
        else:
...
def train(
    ...
    save_model: str = typer.Option("dense_emb_ae_best.keras", help="Path to save best model."),
    ...
    snp_ae = SNP_Autoencoder(
        ...
        mult_2=mult_2, pool_type=pool_type
    )
...
def eval(
    ...
    model_path: str = typer.Option("dense_emb_ae_best.keras", help="Path to trained model (.keras)"),
...
def train_and_eval(
    ...
    save_model: str = typer.Option("dense_emb_ae_best.keras"),
    ...
    run_dir = train(
        ...
        activation=activation, l2_factor=l2_factor, loss=loss, export_model_name=export_model_name, pool_type=pool_type
    )
